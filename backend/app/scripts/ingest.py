...
def calculate_battle_intensity(author_words: int, reviewer_words: int, max_depth: int, num_reviews: int) -> float:
    ...
    if total_words > 0:
        ratio = min(author_words, reviewer_words) / \
            max(author_words, reviewer_words, 1)
        balance_factor = ratio
    else:
        balance_factor = 0.0

    intensity = 0.35 * word_factor + 0.30 * depth_factor + \
        0.20 * review_factor + 0.15 * balance_factor
...
class DataIngester:
    ...
    async def calculate_all_interactions(self):
        ...
        for i, paper_data in enumerate(papers):
            ...
            intensity = calculate_battle_intensity(
                author_words, reviewer_words, max_depth, len(parsed_reviews))
        ...
        logger.info(
            f"Interaction calculation complete: {self.stats['papers_with_interactions']} papers with interactions")
    ...
    async def run(self):
        ...
        logger.info(
            f"Papers with interactions: {self.stats['papers_with_interactions']}")
