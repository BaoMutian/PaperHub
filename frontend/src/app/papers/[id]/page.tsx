...
function RatingCard({ 
  ...
  return (
    <Card className="border-white/10 h-[540px] bg-white/[0.02] backdrop-blur-sm overflow-hidden flex flex-col">
      {/* 平均分展示 - 固定头部 */}
      ...
      {validRatings.length > 1 && (
        <div className="flex-shrink-0 px-6 py-4 bg-white/[0.02] border-t border-white/5">
          <div className="flex justify-between text-[10px] text-white/30 font-mono uppercase tracking-wider">
            <div className="flex flex-col items-center">
              <span className="mb-1 text-emerald-400/50">MAX</span>
              <span className="text-emerald-400 font-bold text-lg">{Math.max(...validRatings.map(r => r.rating)).toFixed(0)}</span>
            </div>
            <div className="w-px h-8 bg-white/5" />
            <div className="flex flex-col items-center">
              <span className="mb-1 text-rose-400/50">MIN</span>
              <span className="text-rose-400 font-bold text-lg">{Math.min(...validRatings.map(r => r.rating)).toFixed(0)}</span>
            </div>
            <div className="w-px h-8 bg-white/5" />
            <div className="flex flex-col items-center">
              <span className="mb-1 text-white/30">STD DEV</span>
